...
function _buildOverviewHTML(fp, qLabel) {
  ...
  <div style="font-family:'Sarabun',sans-serif;width:794px;background:#f4f6f9;padding:0;color:#1c2333;line-height:1.6">
    ...
    <div style="padding:20px 28px">
      ...
      <!-- Budget Progress Bar -->
      <div class="pdf-budget-bar" style="background:#fff;border:1px solid #e4e7ed;border-radius:12px;padding:16px 20px;margin-bottom:18px">
      ...
      <!-- Strategy Table -->
      <div class="pdf-strategy-table" style="background:#fff;border:1px solid #e4e7ed;border-radius:12px;overflow:hidden;margin-bottom:18px">
...
function _buildProjectsHTML(fp, qLabel, strategyFilter) {
  ...
  for (const s of strategies) {
    ...
    html += `
      <!-- Strategy heading -->
      <div class="pdf-strategy-section" style="margin-bottom:14px">
        <div class="pdf-strategy-header" style="background:${color};color:#fff;padding:12px 18px;border-radius:10px 10px 0 0;display:flex;align-items:center;justify-content:space-between">
          <div>
    ...
    ps.forEach((p, idx) => {
      ...
      html += `
        <div class="pdf-project-block" style="background:${rowBg};border-left:4px solid ${color};border-right:1px solid #e4e7ed;border-bottom:1px solid #e4e7ed;padding:14px 18px">
          ...
          <div style="margin-bottom:8px">
            ...
            <div style="font-size:13px;color:#374151;line-height:1.7;padding:0 4px;white-space:pre-wrap;text-align:justify;text-justify:inter-character">${p.result.trim()}</div>
          </div>` : ''}
          ...
          <div style="margin-bottom:8px">
            ...
            <div style="font-size:13px;color:#374151;line-height:1.7;padding:0 4px;white-space:pre-wrap;text-align:justify;text-justify:inter-character">${p.problems.trim()}</div>
          </div>` : ''}
          ...
          <div>
            ...
            <div style="font-size:13px;color:#374151;line-height:1.7;padding:0 4px;white-space:pre-wrap;text-align:justify;text-justify:inter-character">${p.solutions.trim()}</div>
          </div>` : ''}
    ...
    html += `
        <div class="pdf-strategy-footer" style="background:${light};border-left:4px solid ${color};border-right:1px solid #e4e7ed;border-bottom:1px solid #e4e7ed;padding:10px 18px;border-radius:0 0 10px 10px;overflow:hidden">
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  @media print {
    * { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
    ...
    .pdf-strategy-section { break-before: page; page-break-before: always; }
    .pdf-strategy-section:first-child { break-before: avoid; page-break-before: avoid; }
  }
</style>
</head>
<body>${bodyHTML}
...
async function generatePDF() {
  ...
  try {
    ...
    // ── 1. สร้าง HTML ──
    _pdfProgress('📊 กำลังสร้างเนื้อหารายงาน...', 20);
    let bodyHTML = '';
    ...
    // ── 2. เปิด print window ──
    _pdfProgress('🖨️ กำลังเปิดหน้าต่างพิมพ์...', 60);
    const fullHTML = _wrapFullHTML(bodyHTML, fileName);

    const printWin = window.open('', '_blank', 'width=900,height=700');
    if (!printWin) {
      throw new Error('ไม่สามารถเปิด print window ได้ — กรุณาอนุญาต Popup ในเบราว์เซอร์');
    }
    ...
    // ── 3. รอ font + resource โหลดเสร็จแล้วค่อย print ──
    _pdfProgress('⏳ รอ font โหลด...', 80);
    printWin.onload = () => {
      printWin.document.fonts.ready.then(() => {
        setTimeout(() => {
          printWin.focus();
          printWin.print();
          // ปิด window หลัง print dialog ปิด (บาง browser รองรับ)
          printWin.onafterprint = () => printWin.close();
        }, 400);
      });
    };

    _pdfProgress('✅ เปิดหน้าต่างพิมพ์สำเร็จ!', 100);
    setTimeout(() => {
      closePdfModal();
      showToast('🖨️ เลือก "Save as PDF" ใน print dialog เพื่อบันทึกไฟล์ · ' + fileName, 5000);
    }, 800);
