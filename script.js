...
// ===== NEW FIELDS (2569 form update): normalize + migrate legacy data =====
// เพิ่มฟิลด์ใหม่ตามแบบฟอร์มที่หน่วยงานกำหนด: วัตถุประสงค์ / เป้าหมาย (เชิงปริมาณ-คุณภาพ) /
// กิจกรรมและงบประมาณ (ตารางย่อย) / ตัวชี้วัด+ค่าเป้าหมายที่ทำได้+ผลบรรลุ-ไม่บรรลุ
// ฟังก์ชันนี้ทำให้โครงการเก่า (ที่ยังไม่มีฟิลด์ใหม่) ใช้งานต่อได้ปกติ ไม่ต้องกรอกใหม่ทันที
function _kpiObj(k){
  if (k && typeof k === 'object') return { indicator:k.indicator||'', target:k.target||'', achieved:k.achieved||'' };
  return { indicator: k||'', target:'', achieved:'' }; // ข้อมูลเก่า: kpi เป็น string ล้วน
}
function normalizeProject(p){
  p.objectives = Array.isArray(p.objectives) ? p.objectives : [];
  p.goalQuant  = Array.isArray(p.goalQuant) ? p.goalQuant : (p.goalQuant ? [p.goalQuant] : []); // เดิมเป็น string เดี่ยว → แปลงเป็น array 1 รายการอัตโนมัติ
  p.goalQual   = Array.isArray(p.goalQual) ? p.goalQual : [];
  p.activities = Array.isArray(p.activities) ? p.activities.map(a=>({
    name: a.name||'', budget: Number(a.budget)||0, spent: Number(a.spent)||0
  })) : [];
  p.kpi = (p.kpi||[]).map(_kpiObj);
  return p;
}
function normalizeAllProjects(){ projects.forEach(normalizeProject); }
...
const STATUS_LABEL = {done:'แล้วเสร็จ',progress:'อยู่ระหว่างดำเนินการ',pending:'ยังไม่เริ่ม'};
...
function _ltProjectFingerprint(p) {
  return {
    ...
    goalQuant: JSON.stringify(p.goalQuant||[]),
    goalQual: JSON.stringify(p.goalQual||[]),
    activities: JSON.stringify(p.activities||[]),
    kpi: JSON.stringify(p.kpi||[]),
...
function _ltDiff(prev, curr) {
  ...
  (curr||[]).forEach(p => {
    ...
    if (before.objectives !== f.objectives) changes.push({ field:'วัตถุประสงค์', from:'', to:'(อัปเดต)' });
    if (before.goalQuant !== f.goalQuant || before.goalQual !== f.goalQual) changes.push({ field:'เป้าหมาย', from:'', to:'(อัปเดต)' });
    if (before.activities !== f.activities) changes.push({ field:'กิจกรรมและงบประมาณ', from:'', to:'(อัปเดต)' });
    if (before.kpi !== f.kpi) changes.push({ field:'ตัวชี้วัด/ผลการประเมิน', from:'', to:'(อัปเดต)' });
    if (changes.length) {
      ...
      return `<tr class="data-row" onclick="openDetail(${p.id})">
        ...
        <td class="td-actions" onclick="event.stopPropagation()">
          <div class="td-actions-inner">
            ...
            <button class="btn btn-sm btn-icon" title="Export PDF โครงการนี้" onclick="exportSingleProjectPDF(${p.id})" style="color:#dc2626">
...
function openAdd(){
  ...
  editingId=null; tempKPIs=[]; tempImages=[]; tempObjectives=[]; tempGoalQuant=[]; tempGoalQual=[]; tempActivities=[];
  ...
  document.getElementById('goalQuantInput').value='';
  document.getElementById('goalQualInput').value='';
  renderObjectiveList(); renderGoalQuantList(); renderGoalQualList(); renderActivitiesList(); renderKPIList(); renderImgPreview();
...
function openEdit(id){
  ...
  normalizeProject(p);
  editingId=id;
  tempKPIs=(p.kpi||[]).map(_kpiObj);
  tempObjectives=[...(p.objectives||[])];
  tempGoalQuant=[...(p.goalQuant||[])];
  tempGoalQual=[...(p.goalQual||[])];
  tempActivities=(p.activities||[]).map(a=>({...a}));
...
function saveProject(){
  ...
  // ตัดแถวที่ยังไม่ได้กรอกชื่อกิจกรรม/ตัวชี้วัดออกก่อนบันทึก
  const cleanActivities = tempActivities.filter(a=>(a.name||'').trim()!=='' || (a.budget||0)!==0 || (a.spent||0)!==0);
  const cleanKPIs = tempKPIs.map(_kpiObj).filter(k=>k.indicator.trim()!=='' || k.target.trim()!=='' || k.achieved!=='');
  // งบประมาณที่อนุมัติ/ใช้ไป กรอกเองในช่องด้านบนเสมอ (ไม่คำนวณอัตโนมัติจากตาราง 3.1 แล้ว)
  ...
  const data={
    ...
    goalQuant:[...tempGoalQuant],
    goalQual:[...tempGoalQual],
...
function addObjective(){
  const el=document.getElementById('objInput'); const v=el.value.trim(); if(!v) return;
  tempObjectives.push(v); el.value=''; renderObjectiveList();
}
function removeObjective(i){ tempObjectives.splice(i,1); renderObjectiveList(); }
function renderObjectiveList(){
  const el=document.getElementById('objList');
  if(!tempObjectives.length){el.innerHTML='<span style="font-size:11px;color:var(--text3)">ยังไม่มีวัตถุประสงค์</span>';return;}
  el.innerHTML=tempObjectives.map((k,i)=>`<span class="kpi-tag">${escapeHtml(k)}<button onclick="removeObjective(${i})" title="ลบ">×</button></span>`).join('');
}

// ===== เป้าหมายเชิงปริมาณ (2.1) =====
let tempGoalQuant = [];
function addGoalQuant(){
  const el=document.getElementById('goalQuantInput'); const v=el.value.trim(); if(!v) return;
  tempGoalQuant.push(v); el.value=''; renderGoalQuantList();
}
function removeGoalQuant(i){ tempGoalQuant.splice(i,1); renderGoalQuantList(); }
function renderGoalQuantList(){
  const el=document.getElementById('goalQuantList');
  if(!tempGoalQuant.length){el.innerHTML='<span style="font-size:11px;color:var(--text3)">ยังไม่มีเป้าหมายเชิงปริมาณ</span>';return;}
  el.innerHTML=tempGoalQuant.map((k,i)=>`<span class="kpi-tag">${escapeHtml(k)}<button onclick="removeGoalQuant(${i})" title="ลบ">×</button></span>`).join('');
}

// ===== เป้าหมายเชิงคุณภาพ (2.2) =====
let tempGoalQual = [];
function addGoalQual(){
  const el=document.getElementById('goalQualInput'); const v=el.value.trim(); if(!v) return;
  tempGoalQual.push(v); el.value=''; renderGoalQualList();
}
function removeGoalQual(i){ tempGoalQual.splice(i,1); renderGoalQualList(); }
function renderGoalQualList(){
  const el=document.getElementById('goalQualList');
  if(!tempGoalQual.length){el.innerHTML='<span style="font-size:11px;color:var(--text3)">ยังไม่มีเป้าหมายเชิงคุณภาพ</span>';return;}
  el.innerHTML=tempGoalQual.map((k,i)=>`<span class="kpi-tag">${escapeHtml(k)}<button onclick="removeGoalQual(${i})" title="ลบ">×</button></span>`).join('');
}

// ===== 3.1 กิจกรรมและงบประมาณ =====
let tempActivities = [];
function addActivity(){
  tempActivities.push({name:'',budget:0,spent:0});
  renderActivitiesList();
}
function removeActivity(i){ tempActivities.splice(i,1); renderActivitiesList(); }
function updateActivity(i,field,val){
  if(!tempActivities[i]) return;
  tempActivities[i][field] = (field==='name') ? val : (parseFloat(val)||0);
  renderActivitiesList();
}
function _activityTotals(){
  const budget = tempActivities.reduce((a,r)=>a+(Number(r.budget)||0),0);
  const spent  = tempActivities.reduce((a,r)=>a+(Number(r.spent)||0),0);
  return { budget, spent, remain: budget - spent };
}
function renderActivitiesList(){
  ...
  if(!tempActivities.length){
    ...
    body.innerHTML = tempActivities.map((r,i)=>{
      const remain = (Number(r.budget)||0) - (Number(r.spent)||0);
      return `<tr>
        <td style="text-align:center;color:var(--text3)">${i+1}</td>
        <td><input type="text" value="${escapeAttr(r.name)}" placeholder="เช่น จัดเตรียม file ชุดกิจกรรมฯ ต้นฉบับ" onchange="updateActivity(${i},'name',this.value)"></td>
        <td><input type="number" min="0" value="${r.budget||0}" onchange="updateActivity(${i},'budget',this.value)"></td>
        <td><input type="number" min="0" value="${r.spent||0}" onchange="updateActivity(${i},'spent',this.value)"></td>
        <td class="mini-remain" style="${remain<0?'color:var(--red)':''}">${fmt(remain)}</td>
        <td><button type="button" class="mini-remove-btn" onclick="removeActivity(${i})" title="ลบกิจกรรมนี้">×</button></td>
      </tr>`;
    }).join('');
  }
  const t = _activityTotals();
  document.getElementById('actTotalBudget').textContent = fmt(t.budget);
  document.getElementById('actTotalSpent').textContent  = fmt(t.spent);
  document.getElementById('actTotalRemain').textContent = fmt(t.remain);
  // หมายเหตุ: งบประมาณที่อนุมัติ/ใช้ไป (ช่องด้านบน) กรอกเองอิสระเสมอ
  // ไม่ล็อกหรือคำนวณอัตโนมัติจากตารางกิจกรรมนี้แล้วตามที่ผู้ใช้แจ้ง
}

// ===== 3.2 ตัวชี้วัดโครงการ / การวัดและประเมินผล =====
// (ใช้ tempKPIs ตัวเดียวกับที่ประกาศไว้ตอนต้นไฟล์)
function addKPI(){
  tempKPIs.push({indicator:'',target:'',achieved:''});
  renderKPIList();
}
function removeKPI(i){ tempKPIs.splice(i,1); renderKPIList(); }
function updateKPI(i,field,val){ if(!tempKPIs[i]) return; tempKPIs[i][field]=val; }
function renderKPIList(){
  ...
  body.innerHTML = tempKPIs.map((k,i)=>{
    const kk = _kpiObj(k);
    return `<tr>
      <td style="text-align:center;color:var(--text3)">${i+1}</td>
      <td><input type="text" value="${escapeAttr(kk.indicator)}" placeholder="เช่น จำนวนโรงเรียนปลายทางที่ได้รับจัดสรรชุดกิจกรรมฯ" onchange="updateKPI(${i},'indicator',this.value)"></td>
      <td><input type="text" value="${escapeAttr(kk.target)}" placeholder="เช่น ภาคเรียนที่ 1 (7,839 รร. ฯลฯ)" onchange="updateKPI(${i},'target',this.value)"></td>
      <td>
        <select onchange="updateKPI(${i},'achieved',this.value)">
      ...
      <td><button type="button" class="mini-remove-btn" onclick="removeKPI(${i})" title="ลบตัวชี้วัดนี้">×</button></td>
    </tr>`;
  }).join('');
}
function escapeAttr(str){ return (str==null?'':String(str)).replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/</g,'&lt;').replace(/>/g,'&gt;'); }
...
function openDetail(id){
  ...
  if(pdfBtn) pdfBtn.onclick=()=>exportSingleProjectPDF(id);
  const remaining=(p.budget||0)-(p.spent||0)-(p.po||0);
  const kpis=(p.kpi||[]).map(_kpiObj);
  const objectives = p.objectives||[];
  const goalQuant = p.goalQuant||[];
  const goalQual = p.goalQual||[];
  ...
  document.getElementById('detailBody').innerHTML=`
    ...
    ${objectives.length?`
    <div class="detail-section">
      <div class="detail-section-title">1. วัตถุประสงค์</div>
      <div class="kpi-box">${objectives.map(o=>`<div class="kpi-item"><div class="kpi-dot"></div><span>${escapeHtml(o)}</span></div>`).join('')}</div>
    </div>`:''}
    ${(goalQuant.length||goalQual.length)?`
    <div class="detail-section">
      <div class="detail-section-title">2. เป้าหมาย</div>
      ${goalQuant.length?`<div style="font-size:13px;font-weight:600;color:var(--text2);margin-bottom:4px">2.1 เป้าหมายเชิงปริมาณ</div><div class="kpi-box" style="margin-bottom:10px">${goalQuant.map(g=>`<div class="kpi-item"><div class="kpi-dot"></div><span>${escapeHtml(g)}</span></div>`).join('')}</div>`:''}
      ${goalQual.length?`<div style="font-size:13px;font-weight:600;color:var(--text2);margin-bottom:4px">2.2 เป้าหมายเชิงคุณภาพ</div><div class="kpi-box">${goalQual.map(g=>`<div class="kpi-item"><div class="kpi-dot"></div><span>${escapeHtml(g)}</span></div>`).join('')}</div>`:''}
    </div>`:''}
    <div class="detail-section">
      <div class="detail-section-title">3. ผลการดำเนินงานโครงการ</div>
      ${p.result?`<div class="detail-text" style="margin-bottom:12px">${escapeHtml(p.result)}</div>`:''}
      ${activities.length?`
      <div style="font-size:13px;font-weight:600;color:var(--text2);margin:6px 0">3.1 กิจกรรมและงบประมาณ</div>
      ...
      ${kpis.length?`
      <div style="font-size:13px;font-weight:600;color:var(--text2);margin:12px 0 6px">3.2 การวัดและประเมินผล</div>
      <div class="mini-table-wrap"><table class="mini-table">
        <thead><tr><th style="width:34px">ที่</th><th>ตัวชี้วัดโครงการ</th><th style="width:200px">ค่าเป้าหมายที่ทำได้</th><th style="width:130px">ผลการดำเนินงาน</th></tr></thead>
        <tbody>${kpis.map((k,i)=>`<tr><td style="text-align:center;color:var(--text3)">${i+1}</td><td>${escapeHtml(k.indicator)}</td><td>${escapeHtml(k.target)}</td><td>${k.achieved==='yes'?'<span style="color:var(--green);font-weight:600">✅ บรรลุ</span>':k.achieved==='no'?'<span style="color:var(--red);font-weight:600">❌ ไม่บรรลุ</span>':'<span style="color:var(--text3)">— ยังไม่ประเมิน —</span>'}</td></tr>`).join('')}</tbody>
      </table></div>`:''}
      ${(!activities.length && !kpis.length && !p.result)?'<span style="font-size:12px;color:var(--text3)">ยังไม่มีข้อมูลผลการดำเนินงาน</span>':''}
    </div>
    <div class="detail-section">
      <div class="detail-section-title">4. ปัญหาและอุปสรรค</div>
    ...
    <div class="detail-section">
      <div class="detail-section-title">5. แนวทางการพัฒนาหรือแก้ไข / ข้อเสนอแนะ</div>
    ...
    <div class="detail-section">
      <div class="detail-section-title">6. รูปภาพประกอบการดำเนินงาน (${p.images.length} รูป)</div>
...
function _cpCell(txt, opts){
  ...
  // ป้องกันตัวเลขล้นทับช่องข้างเคียงเมื่อคอลัมน์แคบไป: บังคับตัวเลข (right-aligned) ให้อยู่บรรทัดเดียว
  // และตัดด้วย ellipsis แทนการล้นออกนอกช่อง (ไม่ใช้ max-width:0 เพราะจะไปทับ width ที่กำหนดไว้ชัดเจน
  // ในบางตาราง ทำให้ช่องแคบผิดปกติ — overflow:hidden + nowrap ก็เพียงพอเพราะคอลัมน์ถูกกำหนดความกว้าง
  // ตายตัวอยู่แล้วจาก table-layout:fixed)
  const numericSafety = right ? 'overflow:hidden;text-overflow:ellipsis;white-space:nowrap;' : '';
  return `<td ${colspan?'colspan="'+colspan+'"':''} style="border:1px solid #a9b6c9;padding:5px 7px;font-size:${small?'9.5':'10.5'}px;font-weight:${bold?'700':'400'};text-align:${center?'center':right?'right':'left'};background:${bg||'transparent'};color:${color||'#161c26'};${width?'width:'+width+';':''}vertical-align:middle;${numericSafety}">${txt!=null?txt:''}</td>`;
}
...
// กล่องขอบเดียวกับข้อ 3 (ผลการดำเนินงาน) ใช้ครอบรายการแบบ list ให้ดูเป็นชุดเดียวกันทั้งรายงาน
function _cpListBox(items, bg){
  return `<div style="border:1px solid #5a6a85;padding:8px 10px;${bg?`background:${bg};`:''}">
    <ol style="margin:0;padding-left:18px">${items.map(x=>`<li style="margin-bottom:3px">${_cpEsc(x)}</li>`).join('')}</ol>
  </div>`;
}
...
function _buildProjectDetailPageHTML_69(p, year, isLast, noPadding) {
  ...
  const objectivesBlock = objectives.length ? `
    <div class="pdf-subsection-block" style="margin-bottom:10px">
      <div style="font-weight:700;font-size:11px;margin-bottom:4px">1. วัตถุประสงค์</div>
      ${_cpListBox(objectives)}
    </div>` : '';
  const goalQuantBlock = goalQuant.length ? `
    <div class="pdf-subsection-block" style="margin-bottom:8px">
      <div style="font-size:10.5px;font-weight:600;margin-bottom:2px">2.1 เป้าหมายเชิงปริมาณ</div>
      ${_cpListBox(goalQuant)}
    </div>` : '';
  const goalQualBlock = goalQual.length ? `
    <div class="pdf-subsection-block" style="margin-bottom:8px">
      <div style="font-size:10.5px;font-weight:600;margin-bottom:2px">2.2 เป้าหมายเชิงคุณภาพ</div>
      ${_cpListBox(goalQual)}
    </div>` : '';
  const goalBlock = (goalQuant.length || goalQual.length) ? `
    <div style="margin-bottom:10px">
      <div style="font-weight:700;font-size:11px;margin-bottom:4px">2. เป้าหมาย</div>
      ${goalQuantBlock}
      ${goalQualBlock}
    </div>` : '';
  const actTotalBudget = activities.reduce((a,r)=>a+(Number(r.budget)||0),0);
  const actTotalSpent  = activities.reduce((a,r)=>a+(Number(r.spent)||0),0);
  const actTotalRemain = actTotalBudget - actTotalSpent;
  const activitiesBlock = activities.length ? `
    <div class="pdf-subsection-block" style="margin-bottom:10px">
      <div style="font-weight:700;font-size:11px;margin-bottom:4px">3.1 กิจกรรมและงบประมาณ</div>
      <table style="width:100%;border-collapse:collapse;table-layout:fixed">
        <tr>${_cpCell('ที่',{bold:true,center:true,bg:'#e3e9f6',width:'26px'})}${_cpCell('กิจกรรม',{bold:true,bg:'#e3e9f6'})}${_cpCell('งบอนุมัติ',{bold:true,center:true,bg:'#e3e9f6',width:'75px'})}${_cpCell('ใช้ไป',{bold:true,center:true,bg:'#e3e9f6',width:'75px'})}${_cpCell('คงเหลือ',{bold:true,center:true,bg:'#e3e9f6',width:'75px'})}</tr>
        ${activities.map((a,i)=>{const r=(Number(a.budget)||0)-(Number(a.spent)||0);const zebra=i%2===1?'#f6f8fc':'#fff';return `<tr>${_cpCell(i+1,{center:true,bg:zebra})}${_cpCell(_cpEsc(a.name||''),{bg:zebra})}${_cpCell(_cpFmt(a.budget),{right:true,bg:zebra})}${_cpCell(_cpFmt(a.spent),{right:true,bg:zebra})}${_cpCell(_cpFmt(r),{right:true,bold:true,color:r<0?'#dc2626':'#111',bg:zebra})}</tr>`;}).join('')}
        <tr>${_cpCell('รวม',{bold:true,center:true,bg:'#eef2ff',colspan:2})}${_cpCell(_cpFmt(actTotalBudget),{right:true,bold:true,bg:'#eef2ff'})}${_cpCell(_cpFmt(actTotalSpent),{right:true,bold:true,bg:'#eef2ff'})}${_cpCell(_cpFmt(actTotalRemain),{right:true,bold:true,color:actTotalRemain<0?'#dc2626':'#059669',bg:'#eef2ff'})}</tr>
      </table>
    </div>` : '';
  const kpiEvalBlock = kpis.length ? `
    <div class="pdf-subsection-block" style="margin-bottom:10px">
      <div style="font-weight:700;font-size:11px;margin-bottom:4px">3.2 การวัดและประเมินผล</div>
      <table style="width:100%;border-collapse:collapse;table-layout:fixed">
        <tr>${_cpCell('ที่',{bold:true,center:true,bg:'#e3e9f6',width:'26px'})}${_cpCell('ตัวชี้วัดโครงการ',{bold:true,bg:'#e3e9f6'})}${_cpCell('ค่าเป้าหมายที่ทำได้',{bold:true,bg:'#e3e9f6',width:'150px'})}${_cpCell('ผลการดำเนินงาน',{bold:true,center:true,bg:'#e3e9f6',width:'85px'})}</tr>
        ${kpis.map((k,i)=>{const zebra=i%2===1?'#f6f8fc':'#fff';const res=k.achieved==='yes'?'<span style="color:#059669;font-weight:700">บรรลุ</span>':k.achieved==='no'?'<span style="color:#dc2626;font-weight:700">ไม่บรรลุ</span>':'<span style="color:#9aa3b2">—</span>';return `<tr>${_cpCell(i+1,{center:true,bg:zebra})}${_cpCell(_cpEsc(k.indicator||''),{bg:zebra})}${_cpCell(_cpEsc(k.target||''),{bg:zebra})}${`<td style="padding:5px 8px;border:1px solid #5a6a85;text-align:center;background:${zebra}">${res}</td>`}</tr>`;}).join('')}
      </table>
    </div>` : '';
  const images = (p.images || []).map(img => driveImgUrl(img)).filter(Boolean);
  const imagesBlock = images.length ? `
    <div class="pdf-subsection-block" style="margin-bottom:10px">
      <div style="font-weight:700;font-size:11px;margin-bottom:6px">6. รูปภาพประกอบการดำเนินงาน (${images.length})</div>
  ...
  <div style="${isLast?'':'page-break-after:always;'}padding:${noPadding?'0':'6px 36px 28px'};font-size:11px;line-height:1.7">
    ...
    ${objectivesBlock}
    ${goalBlock}
    ${p.result ? `<div class="pdf-subsection-block" style="margin-bottom:10px"><div style="font-weight:700;font-size:11.5px;margin-bottom:4px">3. ผลการดำเนินงานโครงการ</div><div style="border:1px solid #5a6a85;padding:8px 10px;white-space:pre-wrap;text-align:justify">${_cpEsc(p.result).replace(/\n/g,'<br>')}</div></div>` : `<div style="margin-bottom:2px"><div style="font-weight:700;font-size:11.5px;margin-bottom:4px">3. ผลการดำเนินงานโครงการ</div></div>`}
    ${activitiesBlock}
    ${kpiEvalBlock}
    ${p.problems ? `<div class="pdf-subsection-block" style="margin-bottom:10px"><div style="font-weight:700;font-size:11px;margin-bottom:4px">4. ปัญหาและอุปสรรค</div><div style="border:1px solid #5a6a85;padding:8px 10px;background:#fff7f7;white-space:pre-wrap;text-align:justify">${_cpEsc(p.problems).replace(/\n/g,'<br>')}</div></div>` : ''}
    ${p.solutions ? `<div class="pdf-subsection-block" style="margin-bottom:10px"><div style="font-weight:700;font-size:11px;margin-bottom:4px">5. แนวทางการพัฒนาหรือแก้ไข / ข้อเสนอแนะ</div><div style="border:1px solid #5a6a85;padding:8px 10px;background:#f3fbf6;white-space:pre-wrap;text-align:justify">${_cpEsc(p.solutions).replace(/\n/g,'<br>')}</div></div>` : ''}
    ${imagesBlock}
  </div>`;
}
...
const GAS_URL = 'https://script.google.com/macros/s/AKfycbxY7AQRUYiIAl0nr_QywZhDjg2K1Dp6NCYAaoAV6xGaebSZAWiZZEjMzL0XCHMljoU/exec';
let _gasUrl = GAS_URL;
...
// หมายเหตุ: ฟิลด์ใหม่ (objectives/goal_quant/goal_qual/activities) ถูกต่อท้าย
// รายการเดิมไว้เสมอ (ไม่แทรกกลาง) เพื่อไม่ให้ตำแหน่งคอลัมน์ของชีตเก่าที่มีข้อมูล
// อยู่แล้วเลื่อนสลับ — ฟังก์ชัน ensureHeaders() ด้านล่างจะเติมคอลัมน์ใหม่ให้อัตโนมัติ
function getHeaders() {
  return ['id','name','strategy','budget','spent','po','status',
          ...
          'objectives','goal_quant','goal_qual','activities'];
}
...
// รองรับข้อมูล kpi แบบเก่า (ข้อความคั่นด้วย '|') และแบบใหม่ (JSON array of
// {indicator,target,achieved}) — กันชีตเก่าที่ยังไม่ได้แปลงพังตอนอ่านค่ากลับ
function _parseKpiField(raw) {
  if (!raw) return [];
  try {
    const j = JSON.parse(raw);
    if (Array.isArray(j)) return j.map(x => (x && typeof x === 'object') ? x : { indicator: String(x), target: '', achieved: '' });
  } catch (e) {}
  return String(raw).split('|').filter(Boolean).map(x => ({ indicator: x, target: '', achieved: '' }));
}
function _parseJsonArrayField(raw) {
  if (!raw) return [];
  try { const j = JSON.parse(raw); if (Array.isArray(j)) return j; } catch (e) {}
  return [String(raw)]; // เผื่อข้อมูลเก่าที่เคยเก็บเป็นข้อความล้วน (ไม่ใช่ JSON array) — กันพัง
}
...
function rowToObj(row) {
  ...
  obj.objectives = _parseJsonArrayField(obj.objectives);
  obj.goalQuant  = _parseJsonArrayField(obj.goal_quant);
  obj.goalQual   = _parseJsonArrayField(obj.goal_qual);
  obj.activities = _parseJsonArrayField(obj.activities);
  obj.kpi    = _parseKpiField(obj.kpi);
  delete obj.goal_quant; delete obj.goal_qual;
  obj.owner  = obj.owner || '';
...
function objToRow(d) {
  ...
  return [d.id, d.name, d.strategy, d.budget||0, d.spent||0, d.po||0,
          d.status, d.result||'', JSON.stringify(d.kpi||[]),
          ...
          JSON.stringify(d.objectives||[]), JSON.stringify(d.goalQuant||[]), JSON.stringify(d.goalQual||[]),
...
function exportCSV(){
  const headers=['id','name','strategy','budget','spent','po','status',
    'objectives','goal_quant','goal_qual','result','activities','kpi',
    'problems','solutions','quarter','image_count'];
  ...
  const rows=[headers,...projects.map(p=>{
    ...
    return [
      ...
      csvQ((p.objectives||[]).join(' | ')),
      csvQ((p.goalQuant||[]).join(' | ')),
      csvQ((p.goalQual||[]).join(' | ')),
      csvQ(p.result||''),
      csvQ((p.activities||[]).map(a=>`${a.name} (อนุมัติ ${fmt(a.budget)} / ใช้ไป ${fmt(a.spent)})`).join(' | ')),
      csvQ((p.kpi||[]).map(_kpiObj).map(k=>`${k.indicator}${k.target?' ['+k.target+']':''}${k.achieved==='yes'?' (บรรลุ)':k.achieved==='no'?' (ไม่บรรลุ)':''}`).join(' | ')),
      csvQ(p.problems||''),
      csvQ(p.solutions||''),
      p.quarter||'all',
      (p.images||[]).length
...
function _fmtFull(n){ return Math.round(n||0).toLocaleString('th-TH'); }
...
function _buildOverviewHTML(fp, qLabel) {
  ...
  <div style="font-family:'Sarabun',sans-serif;width:794px;background:#fff;padding:0 28px 20px;color:${_INK};font-size:11px;line-height:1.7;box-sizing:border-box">
    ...
    <table style="width:100%;border-collapse:collapse;table-layout:fixed">
      <colgroup>
        <col style="width:34%"><col style="width:8%"><col style="width:15%"><col style="width:15%"><col style="width:14%"><col style="width:14%">
      </colgroup>
      <tr>
        ${_cpCell('ยุทธศาสตร์',{bold:true,center:true,bg:'#e3e9f6',small:true})}
        ${_cpCell('โครงการ',{bold:true,center:true,bg:'#e3e9f6',small:true})}
        ${_cpCell('งบอนุมัติ',{bold:true,center:true,bg:'#e3e9f6',small:true})}
        ${_cpCell('ใช้ไป',{bold:true,center:true,bg:'#e3e9f6',small:true})}
        ${_cpCell('PO',{bold:true,center:true,bg:'#e3e9f6',small:true})}
        ${_cpCell('คงเหลือ',{bold:true,center:true,bg:'#e3e9f6',small:true})}
      </tr>
      ${stratRows}
      <tr>
        ${_cpCell('รวมทั้งหมด',{bold:true,center:true,bg:'#e3e9f6',colspan:2,small:true})}
        ${_cpCell(_fmtFull(totalB),{right:true,bold:true,bg:'#eef2ff',small:true})}
        ${_cpCell(_fmtFull(totalS),{right:true,bold:true,bg:'#eef2ff',small:true})}
        ${_cpCell(_fmtFull(totalPO),{right:true,bold:true,bg:'#eef2ff',small:true})}
        ${_cpCell(_fmtFull(totalR),{right:true,bold:true,color:'#059669',bg:'#eef2ff',small:true})}
      </tr>
    </table>
  </div>`;
}
...
const _PDF_PAGE_CSS = `
  ...
  @media print {
    ...
    /* ── ป้องกันเนื้อหาถูกตัดคาบข้ามหน้า ──
       หมายเหตุ: .pdf-project-block (ทั้งบล็อกโครงการ 1 โครงการ) *ไม่* ใส่ break-inside:avoid
       เพราะบางโครงการมีเนื้อหายาวเกิน 1 หน้ากระดาษ ถ้าบังคับห้ามตัดทั้งบล็อกจะทำให้เบราว์เซอร์
       ต้องผลักทั้งบล็อกไปหน้าถัดไปทั้งก้อน เหลือพื้นที่ว่างเปล่าจำนวนมากในหน้าก่อนหน้า
       (โดยเฉพาะหน้าหัวข้อยุทธศาสตร์ที่มักมีที่ว่างเหลือไม่พอสำหรับทั้งโครงการ) จึงปล่อยให้บล็อกนี้
       ตัดข้ามหน้าได้ตามธรรมชาติ ส่วนองค์ประกอบย่อยภายใน (ตาราง/กล่องข้อความ/หัวโครงการ) ยังคง
       ป้องกันการตัดกลางไว้ตามปกติ */
    #pdfPrintRoot .pdf-project-header,
    #pdfPrintRoot .pdf-subsection-block,
...
/* ─── Export PDF เฉพาะโครงการเดียว (จากปุ่มในตาราง/หน้ารายละเอียด) ───
   ใช้ pipeline การพิมพ์เดียวกับ generatePDF() แต่สร้างแค่ 3 หน้า:
   หน้าปก (ระบุชื่อโครงการ) → รายละเอียดโครงการ (หัวข้อ 1-6 ตามแบบฟอร์มใหม่) → หน้าลงนาม */
async function exportSingleProjectPDF(id){
  const p = projects.find(x=>x.id==id);
  if(!p){ showToast('❌ ไม่พบโครงการนี้', 3000); return; }
  normalizeProject(p);

  try{
    showToast('⏳ กำลังเตรียม PDF โครงการ...', 2500);
    const dateStr = _thaiDateNow();
    const qLabel  = Q_LABEL[p.quarter||'all'];
    const safeName = (p.name||'โครงการ').replace(/[\\/:*?"<>|]+/g,'_').slice(0,60);
    const fileName = `รายงานโครงการ_${safeName}_2569`;

    const pages = [];
    pages.push(`<div class="pdf-page">${_buildCoverHTML('รายโครงการ (เฉพาะโครงการนี้)', p.name||'', dateStr)}</div>`);
    pages.push(`<div class="pdf-page">${_buildProjectDetailPageHTML_69(p, 2569, true, false)}</div>`);
    ...
    const cleanup = () => {
      ...
      showToast('✅ เปิดหน้าต่างพิมพ์แล้ว · เลือก "บันทึกเป็น PDF (Save as PDF)" เพื่อดาวน์โหลด', 5000);
    };
    window.addEventListener('afterprint', cleanup);

    window.print();
    setTimeout(cleanup, 30000); // สำรองไว้เผื่อบางเบราว์เซอร์ไม่ยิง afterprint

  }catch(err){
    console.error('Single Project PDF Error:', err);
    document.body.classList.remove('pdf-printing');
