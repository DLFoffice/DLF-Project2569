...
function openAdd(){
  ...
  editingId=null; tempKPIs=[]; tempImages=[]; tempObjectives=[]; tempGoalQual=[]; tempActivities=[];
  ...
  document.getElementById('fGoalQuant').value='';
  ...
  renderObjectiveList(); renderGoalQualList(); renderActivitiesList(); renderKPIList(); renderImgPreview();
...
function openEdit(id){
  ...
  document.getElementById('fGoalQuant').value=p.goalQuant||'';
...
function saveProject(){
  ...
  const data={
    ...
    goalQuant:document.getElementById('fGoalQuant').value.trim(),
...
function openDetail(id){
  ...
  document.getElementById('detailBody').innerHTML=`
    ...
    ${(p.goalQuant||goalQual.length)?`
    <div class="detail-section">
      <div class="detail-section-title">2. เป้าหมาย</div>
      ${p.goalQuant?`<div style="font-size:13px;font-weight:600;color:var(--text2);margin-bottom:4px">2.1 เป้าหมายเชิงปริมาณ</div><div class="detail-text" style="font-size:14px;margin-bottom:10px">${escapeHtml(p.goalQuant)}</div>`:''}
...
function _buildProjectDetailPageHTML_69(p, year, isLast, noPadding) {
  ...
  const goalBlock = (p.goalQuant || goalQual.length) ? `
    <div style="margin-bottom:10px">
      <div style="font-weight:700;font-size:11px;margin-bottom:4px">2. เป้าหมาย</div>
      ${p.goalQuant?`<div style="font-size:10.5px;font-weight:600;margin-bottom:2px">2.1 เป้าหมายเชิงปริมาณ</div><div style="white-space:pre-wrap;text-align:justify;margin-bottom:6px">${_cpEsc(p.goalQuant).replace(/\n/g,'<br>')}</div>`:''}
...
const GAS_URL = 'https://script.google.com/macros/s/AKfycbwl-Fw3mfm6MIg3TETcO87078N6joFjIbbH_Zn0ZxJ30KYdE-aFnpESm0zG2pSGYvq2OA/exec';
let _gasUrl = GAS_URL;
...
function objToRow(d) {
  ...
  return [d.id, d.name, d.strategy, d.budget||0, d.spent||0, d.po||0,
          ...
          JSON.stringify(d.objectives||[]), d.goalQuant||'', JSON.stringify(d.goalQual||[]),
