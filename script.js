...
function _buildOverviewHTML(fp, qLabel) {
  ...
  const stratRows = [1,2,3,4,5].map(s=>{
    ...
    const barColor = pct>=90?'#b7301f':pct>=70?'#8a5a1f':_S_COLOR[s];
    return `
    <tr style="border-bottom:1px solid #e4e7ed">
      <td style="padding:10px 12px">
        ...
        <span style="font-size:11px;color:#6b7280">${ps.length} โครงการ</span>
      ...
      <td style="padding:10px 12px;text-align:right;font-size:13px;color:#8a5a1f">${_fmtFull(spent)}</td>
      <td style="padding:10px 12px;text-align:right;font-size:13px;color:#5c4a78">${_fmtFull(po)}</td>
      <td style="padding:10px 12px;text-align:right;font-size:13px;color:${rem<0?'#b7301f':'#3a6b4c'};font-weight:600">${_fmtFull(rem)}</td>
      <td style="padding:10px 12px;text-align:center">
        <div style="font-size:11px;display:flex;flex-direction:column;gap:2px">
          <span style="color:#3a6b4c;font-weight:600">✅ ${sd} แล้วเสร็จ</span>
          <span style="color:#2c4a72;font-weight:600">⏳ ${sp} กำลังดำเนิน</span>
          <span style="color:#6b7280;font-weight:600">⭕ ${spe} ยังไม่เริ่ม</span>
        </div>
      </td>
    </tr>`;
  }).join('');
  ...
  return `
  <div style="font-family:'Sarabun',sans-serif;width:794px;background:#ffffff;padding:0;color:${_INK};line-height:1.6">
    ...
    <div style="padding:22px 28px 14px;border-bottom:2px solid ${_NAVY}">
      ...
      <div style="font-size:19px;font-weight:700;color:${_NAVY};margin-bottom:2px">รายงานสรุปผลการดำเนินงานในภาพรวมยุทธศาสตร์</div>
    ...
    <div style="padding:20px 28px">
      ...
      <div style="display:grid;grid-template-columns:repeat(5,1fr);gap:8px;margin-bottom:10px">
        ${_metricCard('#eef1f6','#2c4a72','โครงการทั้งหมด',fp.length+' โครงการ','5 ยุทธศาสตร์',true)}
        ${_metricCard('#eef4ef','#3a6b4c','งบประมาณรวม',_fmtFull(totalB)+' บาท','ที่ได้รับอนุมัติ',true)}
        ${_metricCard('#f8f2e8','#8a5a1f','ใช้จ่ายแล้ว',_fmtFull(totalS)+' บาท',spentPct+'% ของงบรวม',true)}
        ${_metricCard('#f2eef6','#5c4a78','ใช้ไป+PO',_fmtFull(totalS+totalPO)+' บาท',planPct+'% ของงบรวม',true)}
        ${_metricCard(totalR<0?'#f7ece9':'#eef4ef',totalR<0?'#b7301f':'#3a6b4c','คงเหลือสุทธิ',_fmtFull(totalR)+' บาท',remPct+'% ของงบรวม',true)}
      </div>

      <!-- KPI Cards Row 2 -->
      <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:18px">
        ${_metricCard('#eef4ef','#3a6b4c','✅ แล้วเสร็จ',done+' โครงการ',Math.round(done/fp.length*100)+'% ของโครงการทั้งหมด')}
        ${_metricCard('#eef1f6','#2c4a72','⏳ กำลังดำเนิน',prog+' โครงการ',Math.round(prog/fp.length*100)+'% ของโครงการทั้งหมด')}
        ${_metricCard('#f4f5f7','#6b7280','⭕ ยังไม่เริ่ม',pend+' โครงการ',Math.round(pend/fp.length*100)+'% ของโครงการทั้งหมด')}
      </div>
      ...
      <div class="pdf-budget-bar" style="background:#fff;border:1px solid #e4e7ed;border-radius:12px;padding:16px 20px;margin-bottom:18px">
        ...
        <div style="height:16px;border-radius:99px;overflow:hidden;background:#e4e7ed;display:flex;margin-bottom:10px">
          <div style="width:${spentPct}%;background:linear-gradient(90deg,#3a6b4c,#5c8a6d);min-width:${Number(spentPct)>0?2:0}px"></div>
          <div style="width:${planPct}%;background:linear-gradient(90deg,#8a5a1f,#b98a4a);min-width:${Number(planPct)>0?2:0}px"></div>
          <div style="flex:1;background:#e4e7ed"></div>
        </div>
        <div style="display:flex;gap:20px;font-size:12px;flex-wrap:wrap">
          <span style="display:flex;align-items:center;gap:5px"><span style="width:10px;height:10px;border-radius:3px;background:linear-gradient(90deg,#3a6b4c,#5c8a6d);display:inline-block"></span>ใช้ไปแล้ว <strong>${_fmtFull(totalS)} บาท</strong> (${spentPct}%)</span>
          <span style="display:flex;align-items:center;gap:5px"><span style="width:10px;height:10px;border-radius:3px;background:linear-gradient(90deg,#8a5a1f,#b98a4a);display:inline-block"></span>PO ผูกพัน <strong>${_fmtFull(totalPO)} บาท</strong> (${planPct}%)</span>
          <span style="display:flex;align-items:center;gap:5px"><span style="width:10px;height:10px;border-radius:3px;background:#e4e7ed;border:1px solid #ccc;display:inline-block"></span>คงเหลือ <strong style="color:${totalR<0?'#b7301f':'#3a6b4c'}">${_fmtFull(totalR)} บาท</strong> (${remPct}%)</span>
          <span style="margin-left:auto;font-weight:700;color:#2c4a72">งบรวม: ${_fmtFull(totalB)} บาท</span>
        </div>
      </div>
      ...
      <div class="pdf-strategy-table" style="background:#fff;border:1px solid #e4e7ed;border-radius:12px;overflow:hidden;margin-bottom:18px">
        ...
        <table style="width:100%;border-collapse:collapse;font-family:'Sarabun',sans-serif">
          ...
          <tfoot>
            <tr style="background:#eef1f6;border-top:2px solid #2c4a72">
              <td style="padding:10px 12px;font-weight:700;font-size:13px;color:#1c2333">รวมทั้งหมด (${fp.length} โครงการ)</td>
              <td style="padding:10px 12px;text-align:center;font-weight:700;font-size:13px;color:#2c4a72">${planPct}%</td>
              <td style="padding:10px 12px;text-align:right;font-weight:700;font-size:13px">${_fmtFull(totalB)}</td>
              <td style="padding:10px 12px;text-align:right;font-weight:700;font-size:13px;color:#8a5a1f">${_fmtFull(totalS)}</td>
              <td style="padding:10px 12px;text-align:right;font-weight:700;font-size:13px;color:#5c4a78">${_fmtFull(totalPO)}</td>
              <td style="padding:10px 12px;text-align:right;font-weight:700;font-size:13px;color:${totalR<0?'#b7301f':'#3a6b4c'}">${_fmtFull(totalR)}</td>
              <td style="padding:10px 12px;text-align:center">
                <div style="font-size:11px;display:flex;flex-direction:column;gap:2px">
                  <span style="color:#3a6b4c;font-weight:600">✅ ${done} แล้วเสร็จ</span>
                  <span style="color:#2c4a72;font-weight:600">⏳ ${prog} กำลังดำเนิน</span>
                  <span style="color:#6b7280;font-weight:600">⭕ ${pend} ยังไม่เริ่ม</span>
                </div>
              </td>
            </tr>
          </tfoot>
      ...
      html += `
        <div class="pdf-project-block" style="background:${rowBg};border-left:4px solid ${color};border-right:1px solid #e4e7ed;border-bottom:1px solid #e4e7ed;padding:14px 18px">
          ...
          <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:10px">
            ${_miniMetric('งบอนุมัติ', _fmtFull(b)+' บาท', '#2c4a72')}
            ${_miniMetric('ใช้ไป+PO', _fmtFull(spent+po)+' บาท ('+( b>0?Math.round((spent+po)/b*100):0 )+'%)', '#5c4a78')}
            ${_miniMetric('PO ผูกพัน', _fmtFull(po)+' บาท', '#8a5a1f')}
            ${_miniMetric('คงเหลือ', _fmtFull(rem)+' บาท', rem<0?'#b7301f':'#3a6b4c')}
          </div>

          ${p.result && p.result.trim() ? `
          <!-- Result -->
          <div style="margin-bottom:8px">
            <div style="background:#eef1f6;color:#2c4a72;font-size:12px;font-weight:700;padding:5px 10px;border-radius:6px;margin-bottom:5px">📝 ผลการดำเนินงาน</div>
          ...
          <!-- KPIs -->
          <div style="margin-bottom:8px">
            <div style="background:#eef4ef;color:#3a6b4c;font-size:12px;font-weight:700;padding:5px 10px;border-radius:6px;margin-bottom:5px">🎯 ตัวชี้วัด (KPI)</div>
            <div style="padding:0 4px">${p.kpi.map(k=>`<div style="display:flex;align-items:flex-start;gap:7px;font-size:13px;color:#374151;margin-bottom:3px"><span style="width:6px;height:6px;border-radius:50%;background:#3a6b4c;flex-shrink:0;margin-top:5px;display:inline-block"></span>${k}</div>`).join('')}</div>
          </div>` : ''}

          ${p.problems && p.problems.trim() ? `
          <!-- Problems -->
          <div style="margin-bottom:8px">
            <div style="background:#fff0f0;color:#b7301f;font-size:12px;font-weight:700;padding:5px 10px;border-radius:6px;margin-bottom:5px">⚠️ ปัญหาและอุปสรรค</div>
          ...
          <!-- Solutions -->
          <div style="margin-bottom:${(p.images&&p.images.length)?'8px':'0'}">
            <div style="background:#eef4ef;color:#3a6b4c;font-size:12px;font-weight:700;padding:5px 10px;border-radius:6px;margin-bottom:5px">✅ แนวทางแก้ไข</div>
          ...
          <div class="pdf-image-block">
            <div style="background:#eef1f6;color:#2c4a72;font-size:12px;font-weight:700;padding:5px 10px;border-radius:6px;margin-bottom:6px">📸 รูปภาพประกอบการดำเนินงาน (${p.images.length} รูป)</div>
            <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:6px;padding:0 4px">
              ${p.images.map(img => {
                const src = driveImgUrl(img);
                return `<div style="border:1px solid #e4e7ed;border-radius:8px;overflow:hidden;background:#f8fafc;aspect-ratio:4/3">
                  <img src="${src}" crossorigin="anonymous" style="width:100%;height:100%;object-fit:cover;display:block">
                </div>`;
              }).join('')}
            </div>
          </div>` : ''}
    ...
    html += `
        <div class="pdf-strategy-footer" style="background:${light};border-left:4px solid ${color};border-right:1px solid #e4e7ed;border-bottom:1px solid #e4e7ed;padding:10px 18px;border-radius:0 0 10px 10px;overflow:hidden">
          <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:6px 16px;font-size:12px;font-weight:600">
            ...
            <span style="color:#8a5a1f">ใช้จ่าย: <strong>${_fmtFull(totalS)}</strong> บาท</span>
            <span style="color:#5c4a78">PO: <strong>${_fmtFull(totalPO)}</strong> บาท</span>
            <span style="color:${totalR<0?'#b7301f':'#3a6b4c'}">คงเหลือ: <strong>${_fmtFull(totalR)}</strong> บาท</span>
          </div>
        </div>
      </div>
