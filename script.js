...
function saveProject(){
  ...
  // ถ้ามีตาราง 3.1 กิจกรรมและงบประมาณ → ใช้ผลรวมจากตารางนั้นเป็นงบประมาณ/ใช้ไปของโครงการเสมอ
  // ถ้ายังไม่มี (โครงการเก่าที่ยังไม่ได้แปลงข้อมูล) → ใช้ค่าที่กรอกในช่องงบประมาณด้านบนตามเดิม
  const hasActivities = cleanActivities.length > 0;
  const actBudget = cleanActivities.reduce((a,r)=>a+(Number(r.budget)||0),0);
  const actSpent  = cleanActivities.reduce((a,r)=>a+(Number(r.spent)||0),0);

  const data={
    name, strategy:document.getElementById('fStrategy').value,
    budget: hasActivities ? actBudget : (parseFloat(document.getElementById('fBudget').value)||0),
    spent:  hasActivities ? actSpent  : (parseFloat(document.getElementById('fSpent').value)||0),
...
function renderActivitiesList(){
  ...
  // ถ้ามีรายการกิจกรรม → คำนวณงบรวม/ใช้ไปอัตโนมัติจากตารางนี้ (ล็อกฟิลด์บนไม่ให้กรอกซ้ำ)
  const hasRows = tempActivities.length > 0;
  const fBudget = document.getElementById('fBudget'), fSpent = document.getElementById('fSpent');
  document.getElementById('budgetAutoNote').style.display = hasRows ? '' : 'none';
  document.getElementById('spentAutoNote').style.display  = hasRows ? '' : 'none';
  fBudget.readOnly = hasRows; fSpent.readOnly = hasRows;
  if(hasRows){ fBudget.value = t.budget; fSpent.value = t.spent; }
}
...
function _buildProjectDetailPageHTML_69(p, year, isLast, noPadding) {
  ...
  const objectivesBlock = objectives.length ? `
    <div style="margin-bottom:10px"><div style="font-weight:700;font-size:11px;margin-bottom:4px">1. วัตถุประสงค์</div><ol style="margin:0;padding-left:18px">${objectives.map(o=>`<li>${_cpEsc(o)}</li>`).join('')}</ol></div>` : '';
  const goalBlock = (goalQuant.length || goalQual.length) ? `
    <div style="margin-bottom:10px">
      ...
      ${goalQuant.length?`<div style="font-size:10.5px;font-weight:600;margin-bottom:2px">2.1 เป้าหมายเชิงปริมาณ</div><ol style="margin:0;padding-left:18px">${goalQuant.map(g=>`<li>${_cpEsc(g)}</li>`).join('')}</ol>`:''}
      ${goalQual.length?`<div style="font-size:10.5px;font-weight:600;margin-bottom:2px">2.2 เป้าหมายเชิงคุณภาพ</div><ol style="margin:0;padding-left:18px">${goalQual.map(g=>`<li>${_cpEsc(g)}</li>`).join('')}</ol>`:''}
    </div>` : '';
  ...
  <div style="${isLast?'':'page-break-after:always;'}padding:${noPadding?'0':'6px 36px 28px'};font-size:11px;line-height:1.7">
    ...
    ${objectivesBlock}
    ${goalBlock}
    <div style="margin-bottom:2px"><div style="font-weight:700;font-size:11.5px;margin-bottom:4px">3. ผลการดำเนินงานโครงการ</div></div>
    ${p.result ? `<div style="margin-bottom:10px"><div style="border:1px solid #5a6a85;padding:8px 10px;white-space:pre-wrap;text-align:justify">${_cpEsc(p.result).replace(/\n/g,'<br>')}</div></div>` : ''}
    ${activitiesBlock}
    ${kpiEvalBlock}
    ${p.problems ? `<div style="margin-bottom:10px"><div style="font-weight:700;font-size:11px;margin-bottom:4px">4. ปัญหาและอุปสรรค</div><div style="border:1px solid #5a6a85;padding:8px 10px;background:#fff7f7;white-space:pre-wrap;text-align:justify">${_cpEsc(p.problems).replace(/\n/g,'<br>')}</div></div>` : ''}
    ${p.solutions ? `<div style="margin-bottom:10px"><div style="font-weight:700;font-size:11px;margin-bottom:4px">5. แนวทางการพัฒนาหรือแก้ไข / ข้อเสนอแนะ</div><div style="border:1px solid #5a6a85;padding:8px 10px;background:#f3fbf6;white-space:pre-wrap;text-align:justify">${_cpEsc(p.solutions).replace(/\n/g,'<br>')}</div></div>` : ''}
    ${imagesBlock}
  </div>`;
}
